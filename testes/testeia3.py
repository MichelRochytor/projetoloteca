...
def gerar_features_completas(df_jogos, df_times):
    ...
    listas_features = {
        ...
        'Media_GM_Fora': [], 
        'Media_GS_Fora': [],
        'Media_GM_Total_Mandante': [],
        'Media_GS_Total_Mandante': [],
        'Media_GM_Total_Visitante': [],
        'Media_GS_Total_Visitante': []
    }
    ...
    for rodada in range(1, 39):
        ...
        for index, jogo in jogos_da_rodada.iterrows():
            ...
            # Médias totais (independente do mando)
            listas_features['Media_GM_Total_Mandante'].append(
                (stats_m['gols_marcados_casa'] + stats_m['gols_marcados_fora']) / stats_m['jogos'] if stats_m['jogos'] > 0 else 0
            )
            listas_features['Media_GS_Total_Mandante'].append(
                (stats_m['gols_sofridos_casa'] + stats_m['gols_sofridos_fora']) / stats_m['jogos'] if stats_m['jogos'] > 0 else 0
            )
            listas_features['Media_GM_Total_Visitante'].append(
                (stats_v['gols_marcados_casa'] + stats_v['gols_marcados_fora']) / stats_v['jogos'] if stats_v['jogos'] > 0 else 0
            )
            listas_features['Media_GS_Total_Visitante'].append(
                (stats_v['gols_sofridos_casa'] + stats_v['gols_sofridos_fora']) / stats_v['jogos'] if stats_v['jogos'] > 0 else 0
            )
...
def exibir_jogos_por_rodada(df, rodada_desejada):
    ...
    if rodada_desejada == 1:
        ...
        print("pois não há jogos anteriores na temporada para calcular médias.")
    
    # Exemplo específico para Vasco x Flamengo
    ...
    if not jogo_vasco_flamengo.empty:
        ...
        print(f"Vasco (mandante): Média de {jogo['Media_GS_Casa']:.2f} gols sofridos em casa")
        print(f"Flamengo (visitante): Média de {jogo['Media_GS_Fora']:.2f} gols sofridos fora")
