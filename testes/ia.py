import pandas as pd
import numpy as np
from textwrap import shorten # Importa a função para encurtar texto
...
def exibir_jogos_formatado(df, n_amostras=5, largura_maxima=100):
    print(f"\n--- Exibindo uma amostra de {n_amostras} jogos ---")
    df_sample = df.sample(min(n_amostras, len(df)), random_state=42).copy()
    ...
    # Ajusta a largura máxima dinamicamente
    ...
    print("\n" + "=" * largura_total)
    print("JOGOS DO BRASILEIRÃO COM FEATURES".center(largura_total))
    print("=" * largura_total)
    ...
    print("\n" + "-" * largura_total)
    print("ESTATÍSTICAS MÉDIAS GERAIS (da planilha completa):")
    # Pequeno ajuste aqui para usar o dataframe completo (df) para as médias
    print(f"Média de Gols (Mandante): {df['Media_GM_Casa'].mean():.2f} marcados vs {df['Media_GS_Casa'].mean():.2f} sofridos")
    print(f"Média de Gols (Visitante): {df['Media_GM_Fora'].mean():.2f} marcados vs {df['Media_GS_Fora'].mean():.2f} sofridos")
    print("=" * largura_total)
...
# Uso da sua função para exibir a tabela final
exibir_jogos_formatado(df_serie_a_enriquecido, n_amostras=10)