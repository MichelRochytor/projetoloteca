...
def gerar_features_completas(df_jogos, df_times):
    ...
    stats_times = {time: {
        ...
        'jogos_fora': 0,
        'ultimos_5_saldos': deque(maxlen=5),  # Saldo dos últimos 5 jogos
    ...
    listas_features = {
        ...
        'Saldo_GM_Fora': [],
        'Saldo_GS_Fora': [],
        'Saldo_Ultimos_5_Mandante': [],
        'Sequencia_5_Mandante': [],
        'Saldo_Ultimos_5_Visitante': [],
    ...
    for rodada in range(1, 39):
        ...
        for index, jogo in jogos_da_rodada.iterrows():
            ...
            # Saldo dos últimos 5 jogos
            saldo_ultimos_5_m = sum(stats_m['ultimos_5_saldos'])
            saldo_ultimos_5_v = sum(stats_v['ultimos_5_saldos'])
            listas_features['Saldo_Ultimos_5_Mandante'].append(saldo_ultimos_5_m)
            listas_features['Saldo_Ultimos_5_Visitante'].append(saldo_ultimos_5_v)
            ...
            # Atualiza as listas de últimos jogos
            stats_m['ultimos_5_saldos'].append(saldo_m)
            stats_m['ultimos_5_resultados'].append(resultado_m)
            stats_v['ultimos_5_saldos'].append(saldo_v)
...
def exibir_jogos_por_rodada(df, rodada_desejada):
    ...
    col_config = {
        ...
        'Saldo_Ultimos_5_Mandante': {'nome': 'S5(M)', 'largura': 6, 'alinhamento': '>'},
        'Sequencia_5_Mandante': {'nome': 'Seq5(M)', 'largura': 8, 'alinhamento': '^'},
        'Saldo_Ultimos_5_Visitante': {'nome': 'S5(V)', 'largura': 6, 'alinhamento': '>'},
    ...
    if rodada_desejada == 1:
        ...
        print("pois não há jogos anteriores na temporada para calcular saldos e sequências.")
    elif rodada_desejada < 6:
        print(f"\n📝 NOTA: Para a Rodada {rodada_desejada}, algumas estatísticas podem estar baseadas em menos de 5 jogos anteriores.")
    ...
    if not jogo_vasco_flamengo.empty:
        ...
        print(f"Vasco (mandante): Saldo últimos 5 jogos: {jogo['Saldo_Ultimos_5_Mandante']}, Sequência: {jogo['Sequencia_5_Mandante']}")
        print(f"Flamengo (visitante): Saldo últimos 5 jogos: {jogo['Saldo_Ultimos_5_Visitante']}, Sequência: {jogo['Sequencia_5_Visitante']}")
